...
import { useAnalyzeImage, getGetAnalysisHistoryQueryKey, getGetAnalysisStatsQueryKey } from "@workspace/api-client-react";
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      await analyzeMutation.mutateAsync({
        data: { imageBase64: base64, mimeType: selectedImage.file.type, notes: notes.trim() || undefined }
      });
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div><h3 className="font-medium text-sm mb-2">Observed Indicators</h3>
                    <ul className="space-y-1.5">{analyzeMutation.data.indicators.map((ind, i) => <li key={i} className="text-sm text-muted-foreground flex items-start gap-2"><span className="mt-1 w-1.5 h-1.5 rounded-full bg-primary/50 shrink-0" />{ind}</li>)}</ul>
                  </div>
                  <div><h3 className="font-medium text-sm mb-2">Recommended Actions</h3>
                    <ul className="space-y-1.5">{analyzeMutation.data.recommendations.map((rec, i) => <li key={i} className="text-sm text-muted-foreground flex items-start gap-2"><CheckCircle2 className="w-4 h-4 text-primary shrink-0 mt-0.5" />{rec}</li>)}</ul>
                  </div>
                </div>
